...
# nyk510さん　https://github.com/nyk510/atmacup10/blob/master/src/exp__027.py
    ...
    def create_mapping(self, input_df, y):
        ...
        for col in self.columns:
            ...
            for idx_train, idx_valid in self.cv:
                ...
                _df = _df.reidenx(keys)
                _df = _df.fillna(_df.mean())
                oof[idx_valid] = input_df[col][idx_valid].map(_df.to_dict())

            output_df[col] = oof

            self.mapping_df_[col] = target.groupby(X).mean()

        return output_df

    def fit(self, input_df: pd.DataFrame, y=None, **kwargs):
...
def main():
    ...
    # TargetEncodingBlock用のfold, cv
    fold = KFold(n_splits=5, shuffle=True, random_state=2021)
    te_cv = list(fold.split(train_df))
