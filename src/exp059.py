...
def preprocess(input_df):
    ...
    output_df["半期"] = output_df["取引時点_年次"].str.split("年", expand=True)[1]
...
class LandPriceBlock(BaseBlock):
    ...
    def transform(self, input_df):
        ...
        land_price_df = pd.read_csv("../add/landprice.csv",
                                    encoding="cp932",
...
# 取引事情があるかどうかのフラグ
def get_circumstance(input_df):
    output_df = pd.DataFrame()
    output_df["is_circumstance"] = input_df["取引の事情等"].isnull() * 1
    return output_df


# 接続駅数をカウントする
def junction_count(input_df):
    station_df = pd.read_csv("../add/station.csv")
    junction_df = station_df["最寄駅：名称"].value_counts().reset_index().rename(columns={"index": "最寄駅：名称",
                                                                                    "最寄駅：名称": "接続駅数"})

    _df = pd.merge(input_df, junction_df, on="最寄駅：名称", how="left")
...
class MeanPriceAggBlock(BaseBlock):
    ...
    def fit(self, input_df, y=None):
        ...
        if self.split_year is None:
            ...
            _input_df = input_df[input_df["取引時点"]>(2021-self.split_year)].copy()
        ...
        output_encoded = np.zeros((len(input_df), len(self.agg_funcs)))
        kf = KFold(n_splits=5, shuffle=True, random_state=20000)
        for index, agg_func in enumerate(self.agg_funcs):
            for train_idx, valid_idx in kf.split(input_df):
...
def main():
    ...
    process_blocks = [
        ...
        *[LandAggregationBlock(whole_df=whole_df, key="最寄駅：名称", agg_column=c,
        ...
        WrapperBlock(get_circumstance),
        WrapperBlock(junction_count),
        *[MeanPriceAggBlock(key=c, agg_funcs=["mean", "median", "std", "skew"], split_year=3) for c in [
            "都道府県名",
            "市区町村名",
            "地区名",
            "最寄駅：名称",
        ]],
        *[MeanPriceAggBlock(key=c, agg_funcs=["mean", "median", "std", "skew"], split_year=5) for c in [
            "都道府県名",
            "市区町村名",
            "地区名",
            "最寄駅：名称",
        ]],
        *[MeanPriceAggBlock(key=c, agg_funcs=["mean", "median", "std", "skew"]) for c in [
            "都道府県名",
            "市区町村名",
            "地区名",
            "最寄駅：名称",
