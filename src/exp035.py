...
# 緯度経度から地域で最も平均価格が高い場所からの距離を算出
class DistanceBlock(BaseBlock):
    def __init__(self, whole_df: pd.DataFrame):
        self.whole_df = whole_df

    def fit(self, input_df, y=None):
        geo_df = pd.read_csv("../add/geo_info.csv")
        merge_geo = pd.merge(self.whole_df, geo_df, on=["都道府県名", "市区町村名", "地区名"], how="left")
        _geo = merge_geo.groupby(["都道府県名", "市区町村名", "地区名"]).mean()[["取引価格（総額）_log",
                                                                    "latitude",
                                                                    "longitude"]].reset_index().groupby(
            ["都道府県名", "市区町村名"]).agg({"取引価格（総額）_log": "max"}).reset_index()

        _df = merge_geo.groupby(["都道府県名", "市区町村名"]).mean()[["latitude", "longitude"]].reset_index()

        _geo_df = pd.merge(_df, _geo, on=["都道府県名", "市区町村名"], how="left").drop_duplicates("市区町村名")

        self.geo_dict = {}
        for i in range(len(_geo_df)):
            self.geo_dict[_geo_df.loc[i]["市区町村名"]] = (_geo_df.loc[i]["latitude"], _geo_df.loc[i]["longitude"])
    ...
    def get_distances(self, x1, x2, region):
        if region in self.geo_dict:
            master = self.geo_dict[region]
            if np.isnan(x1):
                return np.nan
            else:
    ...
    def transform(self, input_df):
        ...
        output_df["Area_Distance"] = output_df[["latitude", "longitude", "市区町村名"]].apply(
            lambda x: self.get_distances(x[0], x[1], x[2]), axis=1)

        return output_df["Area_Distance"]
...
def main():
    ...
    train_df = pd.read_csv(os.path.join(config.INPUT, "train_clip.csv"))
    ...
    process_blocks = [
        ...
        *[AggregationBlock(whole_df=whole_df,
                           key="最寄駅：名称",
                           agg_column=c,
                           agg_funcs=["mean", "std", "max", "min", "count"]) for c in ['最寄駅：距離（分）',
                                                                                       '面積（㎡）',
                                                                                       '建ぺい率（％）',
                                                                                       '容積率（％）']],
        *[AggregationBlock(whole_df=whole_df,
        ...
        *[AggregationBlock(whole_df=whole_df,
                           key="最寄駅：距離（分）",
                           agg_column=c,
                           agg_funcs=["mean", "std", "max", "min", "count"]) for c in ['面積（㎡）',
                                                                                       '建ぺい率（％）',
                                                                                       '容積率（％）']],
        ...
        *[LandAggregationBlock(whole_df=whole_df, key="最寄駅：名称", agg_column=c,
        ...
        GeoCodeBlock(),
        DistanceBlock(whole_df=whole_df)
    ]
